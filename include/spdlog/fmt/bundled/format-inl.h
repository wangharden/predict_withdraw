...
FMT_FUNCTION void assert_fail(const char* file, int line, const char* message) {
...
FMT_FUNCTION int safe_strerror(int error_code, char*& buffer,
...
FMT_FUNCTION void format_error_code(internal::buffer<char>& out, int error_code,
...
FMT_FUNCTION void fwrite_fully(const void* ptr, size_t size, size_t count,
                           FILE* stream) {
...
FMT_FUNCTION void report_error(format_func func, int error_code,
                           string_view message) FMT_NOEXCEPT {
...
template <typename Char> FMT_FUNCTION std::string grouping_impl(locale_ref loc) {
  return std::use_facet<std::numpunct<Char>>(loc.get<std::locale>()).grouping();
}
template <typename Char> FMT_FUNCTION Char thousands_sep_impl(locale_ref loc) {
  return std::use_facet<std::numpunct<Char>>(loc.get<std::locale>())
      .thousands_sep();
}
template <typename Char> FMT_FUNCTION Char decimal_point_impl(locale_ref loc) {
  return std::use_facet<std::numpunct<Char>>(loc.get<std::locale>())
      .decimal_point();
}
}  // namespace internal
...
FMT_FUNCTION std::string internal::grouping_impl(locale_ref) {
  return "\03";
}
template <typename Char>
FMT_FUNCTION Char internal::thousands_sep_impl(locale_ref) {
  return FMT_STATIC_THOUSANDS_SEPARATOR;
}
template <typename Char>
FMT_FUNCTION Char internal::decimal_point_impl(locale_ref) {
  return '.';
}
#endif

FMT_API FMT_FUNCTION format_error::~format_error() FMT_NOEXCEPT = default;
FMT_API FMT_FUNCTION system_error::~system_error() FMT_NOEXCEPT = default;

FMT_FUNCTION void system_error::init(int err_code, string_view format_str,
                                 format_args args) {
...
template <> FMT_FUNCTION int count_digits<4>(internal::fallback_uintptr n) {
...
FMT_FUNCTION fp get_cached_power(int min_exponent, int& pow10_exponent) {
...
FMT_FUNCTION internal::utf8_to_utf16::utf8_to_utf16(string_view s) {
...
FMT_FUNCTION internal::utf16_to_utf8::utf16_to_utf8(wstring_view s) {
...
FMT_FUNCTION int internal::utf16_to_utf8::convert(wstring_view s) {
...
FMT_FUNCTION void windows_error::init(int err_code, string_view format_str,
                                  format_args args) {
  error_code_ = err_code;
  memory_buffer buffer;
...
FMT_FUNCTION void internal::format_windows_error(internal::buffer<char>& out,
                                             int error_code,
                                             string_view message) FMT_NOEXCEPT {
  FMT_TRY {
    wmemory_buffer buf;
...
FMT_FUNCTION void format_system_error(internal::buffer<char>& out, int error_code,
                                  string_view message) FMT_NOEXCEPT {
  FMT_TRY {
    memory_buffer buf;
    buf.resize(inline_buffer_size);
...
FMT_FUNCTION void internal::error_handler::on_error(const char* message) {
  FMT_THROW(format_error(message));
}

FMT_FUNCTION void report_system_error(int error_code,
                                  fmt::string_view message) FMT_NOEXCEPT {
  report_error(format_system_error, error_code, message);
}

#if FMT_USE_WINDOWS_H
FMT_FUNCTION void report_windows_error(int error_code,
                                   fmt::string_view message) FMT_NOEXCEPT {
  report_error(internal::format_windows_error, error_code, message);
}
#endif

FMT_FUNCTION void vprint(std::FILE* f, string_view format_str, format_args args) {
  memory_buffer buffer;
...
FMT_FUNCTION void vprint(string_view format_str, format_args args) {
