...
enum TDF_MSG_ID
{
	...
	MSG_DATA_NB_TRANSACTION,        //新债券逐笔成交
	MSG_DATA_NB_ORDER,              //新债券逐笔委托
	MSG_DATA_BBQSNAP,				//BBQSnap BBQ债券快照数据
	MSG_DATA_BBQSTEP,				//BBQSTEP BBQ债券逐笔成交
...
struct TDF_CODE
{
    ...
    char szENName[48];
    char szCNName[64];      //chinese name: 沪银1302
    int nType;              //代码类型               
...
struct TDF_NoSSE_Bond{
	...
	int		nBidQuoteTime;		 //买入报价时间
	int		nAskQuoteTime;		 //卖出报价时间
};
...
{
	int nPreWeightedCleanPrice;   //前加权平均净价
	int nWeightedCleanPrice;      //加权平均净价
	int nPreClosingYield;         //前收盘收益率
	int nOpeningYield;            //开盘收益率
	int nHighestYield;            //最高收益率
	int nLowestYield;             //最低收益率
	int nPreWeightedYield;        //前加权平均收益率
	int nWeightedYield;           //加权平均收益率
	int nLastYield;				  //最新收益率
	char chSecurityStatus;		  //固定收益N: 正常, S:挂起(禁止) Z:摘牌 D: 到期
	char chSecurityType;		  //固定收益 N：国债,	E：公司债 P：私募债券 A：信贷资产支持证券 T：特定债券 R：公募 REITs 空格：其他
	char chCanSellShort;		  //固定收益 Y：可以做空 N：不能做空
	char chTradeProduct[2];		  //固定收益 01：国债现券 03：公司债现券 05：债券质押式协议回购 06：中小企业私募债券 10：三方回购 15：公募 REITs 21：特定债券
	int nBasicInterestRateSpread; //固定收益 基本利差, 单位为厘
};

//深圳新债券字段
struct TDF_NB_DATA
{
	int				nHighLimit[5];			//5种交易方式的涨停价
	int				nLowLimit[5];			//5种交易方式的跌停价
	unsigned char	nPricingMethod;			//1=净价  2=全价
	...
	__int64			iPriceOpen;				//非匹配类型开盘价
	__int64			iPriceHigh;				//非匹配类型最高价
	__int64			iPriceLow;				//非匹配类型最低价
};
...
struct TDF_MARKET_DATA
{
	...
	SSE_BOND_DATA SSEBondData;			//上证固收债券相关字段
	TDF_NB_DATA NBData;					//深圳新债券字段
...
//数据类型MSG_DATA_NB_TRANSACTION
struct TDF_NBTRANSACTION
{
	TDF_TRANSACTION trans;			//逐笔成交常规通用字段
	...
	__int64 iNetPrice;				//成交净价
	__int64 iFullPrice;				//成交全价
	__int64 iAccruedInterest;   	//应付利息
	__int64 iYieldToMaturity;       //到期收益率
	__int64 iTradeAmount;			//成交金额
};
//数据类型MSG_DATA_ORDER
struct TDF_ORDER
{
...
//数据类型MSG_DATA_NB_ORDER
struct TDF_NBORDER
{
	TDF_ORDER order;				//逐笔委托常规通用字段
...
enum TDF_ENVIRON_SETTING
{
	...
	TDF_ENVIRON_JNI_MAX_SIZE,				//Java消息队列大小，默认为20万，0为无限制
};
//双活模式设置
...
{ 
	int nTime;                  //报价时间，单位为秒
	__int64 iPrice;				//价格，原始值*10000
	__int64 iYield;				//收益率,原始值*10000
	__int64 iDirtyPrice;		//全价,原始值*10000
	__int64 iNetPrice;			//净价,原始值*10000
	__int64 iVolumeTotal;		//报价总量
	char chQuoteId[64];         //报价ID
	char chPriceDesc[64];		//报价描述
	char chMemo[512];			//报价原始备注
	char chVolumeValue[120];	//报价量描述
	char chPriceType;			//报价类型
	__uint8 uExeciseFlag;       //行权标志
	__uint8 uBargainFlag;		//可议价标志
	__uint8 uRelationFlag;		//打包和OCO标志
};

struct TDF_BBQEX_INDEX
{
	int nTime;					//时间，单位为秒
	int nTimeType;				//时点类型，-1无效
	char chQuoteId[64];			//报价ID
	__int64 nMatch;				//指数值
};

//BBQ 行情快照 MSG_DATA_BBQSNAP
struct TDF_BBQEx_Snap
{
	char szWindCode[32];		//Wind Code: AG1302.SHF
	char chName[64];			//BBQ简称
	char chBench[16];			//基准期限
	char chIssuer[32];			//发行人
	int nType;					//类型
	int	nSourceCode;			//中介机构：国际001，国利002，中诚003，平安004，信唐005，上田006
	int nDate;                  //交易日期，年月日
	int nTime;                  //TD接收报价数据的最新本地时间，HHMMSSsss
	TDF_BBQEX_ASKBID stAsk;     //卖报价信息
	TDF_BBQEX_ASKBID stBid;     //买报价信息
	TDF_BBQEX_INDEX  stIndex;	//Index
};

//BBQ逐笔成交 MSG_DATA_BBQSTEP
struct TDF_BBQEx_Deal
{
	...
	int nDate;						//交易日期，年月日
	int nTime;						//成交时间，单位为秒
	char chDealId[64];				//成交ID
	__int64 iTradePrice;			//成交价,原始值*10000
	__int64 iYieldRate;				//成交收益率,原始值*10000
	__int64 iNetPrice;				//成交净价,原始值*10000
	__int64 iDirtyPrice;			//成交全价,原始值*10000
	__int64 iVolume;				//成交量	
	char chSettlSpeed[56];			//清算速度
	char chPriceType;				//成交价类型  1：净价；2：全价；3：收益率
	__uint8 uExerciseFlag;			//行权标志  若存在“行权”字样则赋值为0   若存在“到期”字样则赋值为1
	__uint8 uStatus;                //成交类型 1:done  2:gvn  3:tkn  4:默认  6:ref撤销
	__uint8 uDelFlag;				//删除标识  0:正常  1:删除    
};
