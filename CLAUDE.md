...
1. **先写文档**：在 `docs/` 增补本轮 `design-vX-*.md`（明确“现状/变更点/口径/验收点”）
2. **再改代码并验证**：实现与回归验证通过后，必要时反向补充文档细节
3. **维护索引**：在 `CLAUDE.md` 的“目录结构/设计文档索引”追加对应条目
...
交易所/柜台在若干关键时点会发生“批量推送/状态切换”，此时策略应避免发单，至少留出 **1 秒缓冲**（从 `+1s` 开始再下单）：

- `09:15:00`：盘前单推送到交易所（集合竞价阶段）
- `09:30:00`：上午开盘/连续竞价开始
- `13:00:00`：下午开盘

推荐门控口径（本机时间 `hhmmss`）：

- 最早下单时间：`09:15:01` / `09:30:01` / `13:00:01`

> 该约束适用于所有下单路径（批量预挂、盘中触发、跟单等）；如需在这些时点附近调度，优先改为 “等 1 秒再执行”。

---

## 下单限频约束（必须遵守）

- **1 秒内委托总量不超过 150**（所有下单路径合并统计：批量预挂、盘中触发、跟单等）
...
└── docs/                    ← 设计文档 & 参考资料
    ...
    ├── design-v7-盘前0915批量预挂风控限制.md  ← 第 7 轮：盘前 09:15 前委托总笔数上限（200）
...
9. [design-v7](docs/design-v7-盘前0915批量预挂风控限制.md) — 盘前 09:15 前委托总笔数上限（200）
